...
def generate_install_script(dependencies, llama_stack_install):
    """Generate install-deps.sh script with all pip install commands."""
    ...
    # Convert RUN commands to shell commands
    for line in dependencies.splitlines():
        line = line.strip()
        if line.startswith("RUN "):
            lines.append(line[4:])  # Strip "RUN " prefix
        elif line:
            lines.append(line)

    lines.append("")

    # Add llama-stack install
    if llama_stack_install:
        for line in llama_stack_install.splitlines():
            line = line.strip()
            if line.startswith("RUN "):
                lines.append(line[4:])
            elif line:
...
def main():
    ...
    generate_install_script(dependencies, llama_stack_install)
